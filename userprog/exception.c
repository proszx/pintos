...
kill (struct intr_frame *f)
{
  ...
  switch (f->cs)
    ...
    case SEL_UCSEG:
      ...
      intr_dump_frame (f);
      _exit (-1); // terminate. no more wait, parent
...
page_fault (struct intr_frame *f)
{
  ...
  void* esp = user ? f->esp : curr->cur_esp;
  ...
  if (on_stack_frame && is_stack_addr) {
    ...
    if (pgtab_entry(curr->page_table, fault_page) == false)
     pgtab_loadzpage (curr->page_table, fault_page);
  }

  if(!pgtab_loadpage(curr->page_table, curr->pdir, fault_page) ) {
    goto PAGE_FAULT_VIOLATED_ACCESS;
  }
